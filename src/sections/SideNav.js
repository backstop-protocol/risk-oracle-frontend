  ...
  return (
    <Box sx={{ width: "7%", height: "100%", position: "sticky", top: 0, left: 0, display:"flex", flexDirection:"column" }}>
      ...
      <Paper sx={{height:"100%", minHeight:"92vh", maxHeight:"92vh"}}>
        ...
        >
          {assets.map(_ => <Tab sx={{justifyContent:"start", minHeight:"35px"}} key={_.name} value={_.name} iconPosition="start" icon={<Box component="img" sx={{ height: "3vh" }} alt={`${_.name} icon`} src={`/asset-icons/${_.name}.webp`} />} label={_.name} />)}
        </Tabs>
      </Paper>
    </Box>
  )
